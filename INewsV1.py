...
class INewsV1:
    ...
    def GetPathSDI(self, selenium:None) -> str:
        ...
        # page = requests.get('https://embed.rctiplus.com/live/inews/inewsid', headers=self.custom_headers)
        # soup = BeautifulSoup(page.text, "html.parser")

        # if soup.find('script', string=re.compile('auth_key')):
        #     token = soup.find('script', string=re.compile('auth_key')).text
            
        #     token = token.partition("v1/live-event/4/url')")[2].partition("console.log('")[2].partition("')")[0]
        
        page = json.loads(token.text)
        print(page)
        exit()
        print(page['data']['url'])
        token_1 = page['data']['url'].partition('hdnts=exp=')[2].partition('~hmac=')[0]
        token_2 = page['data']['url'].partition('~hmac=')[2]
        path_uri ='https://icdn.rctiplus.id/inews-sdi.m3u8?auth_key='+ token_1 +'-'+token_2
        print(path_uri)
        response = HTTPRequest("get", path_uri, self.custom_headers).Hit()
