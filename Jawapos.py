...
from libs.VideoProsessorJawapos import VideoProsessor
        ...
        try:
            while self.start_process:
                if playlist_uri is not None:
                    ...
                    if status_ts:
                        now_filename = F"JAWAPOSSTREAMING_{datetime.datetime.now().strftime('%m-%d-%H-%M-%S')}"
...
if __name__ == "__main__":
    ...
    JawaPos = JawaPos(
        ...
        converter_host=CONFIG.SOCKET_SERVER_JAWAPOS["HOST"],
        converter_port=CONFIG.SOCKET_SERVER_JAWAPOS["PORT"],
        buffer_size=CONFIG.SOCKET_SERVER_JAWAPOS["BUFFER_SIZE"],
