...
from libs.VideoProsessorPonTv import VideoProsessor
...
class JawaPos:
    ...
    def GetPlaylist(self) -> str:
        """Ambil playlist utama (master) lalu lanjut ke variant (chunklist)."""
        ...
        # Coba parse pakai m3u8
        try:
            m3u8_data = m3u8.loads(text).data
        except Exception as e:
            logging.error(f"Gagal parsing playlist: {e}")
            return None

        # Kalau ada 'playlists' (HLS master)
        if "playlists" in m3u8_data and len(m3u8_data["playlists"]) > 0:
            uri = m3u8_data["playlists"][0]["uri"]
            if not uri.startswith("http"):
                uri = f"{self.host_directory}{uri}"
            logging.info(f"Get Playlist Success (master → variant: {uri})")
            return uri

        # Kalau tidak ada 'playlists', coba cari manual baris chunklist
        for line in text.splitlines():
            if "chunklist" in line and line.strip().endswith(".m3u8"):
                uri = line.strip()
                if not uri.startswith("http"):
                    uri = f"{self.host_directory}{uri}"
                logging.info(f"Get Playlist Success (fallback chunklist: {uri})")
                return uri

        # Kalau ternyata sudah variant (punya segments)
        if "segments" in m3u8_data and len(m3u8_data["segments"]) > 0:
            logging.info("Get Playlist Success (variant langsung)")
            return url

        logging.error("Playlist tidak berisi segmen / variant.")
        logging.debug(f"Isi mentah playlist:\n{text[:400]}")
        return None
        ...
        try:
            while self.start_process:
                if playlist_uri is not None:
                    ...
                    if status_ts:
                        now_filename = F"PONTV_{datetime.datetime.now().strftime('%m-%d-%H-%M-%S')}"
...
if __name__ == "__main__":
    ...
    JawaPos = JawaPos(
        ...
        converter_host=CONFIG.SOCKET_SERVER_PONTV["HOST"],
        converter_port=CONFIG.SOCKET_SERVER_PONTV["PORT"],
        buffer_size=CONFIG.SOCKET_SERVER_PONTV["BUFFER_SIZE"],
