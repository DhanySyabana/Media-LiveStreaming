...
class INewsV1:
    ...
    def GetPathSDI(self, selenium:None) -> str:
        path_uri = None
        token = requests.get('https://api.rctiplus.com/api/v1/live-event/1/url?appierid=undefined', headers={'Authorization':'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJ2aWQiOjAsInRva2VuIjoiYTQ3MDA0ZTk2ODRkNDBkOSIsInBsIjoid2ViIiwiZGV2aWNlX2lkIjoiMTYwZWZiYmMtMDQ0My0xMWVlLTk0YmUtMDAxNjNlMDM4OGRiIn0.f1zienRzzc0rKo1Mt_BxSqTAaAGxF0Dzmhnx7oJOcbg'})
        ...
        try:
            while self.start_process:
                if playlist_uri is not None:
                    ...
                    if status_ts:
                        now_filename = F"RCTISTREAMING_{datetime.datetime.now().strftime('%m-%d-%H-%M-%S')}"
