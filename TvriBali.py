...
from libs.VideoProsessorTvriBali import VideoProsessor
    ...
    def GetSegment(self) -> list:
        ...
        if response.status_code == 200:
            ...
            for segment in segments:
                ...
                # https://ott-balancer.tvri.go.id/live/eds/Bali/_shared_c85e6b3aea6b9f455325b1a281dad8e5/Bali-avc1_250000=10000-mp4a_64000=20002-begin=3215984400000000-dur=100000000-seq=32159845.ts
                # https://ott-balancer.tvri.go.id/live/eds/Bali/_shared_c85e6b3aea6b9f455325b1a281dad8e5/Bali-avc1_500000=10001-mp4a_64000=20002-begin=3215983500000000-dur=100000000-seq=32159836.ts
                print(F'{self.host_directory_ts}/{segment["uri"].replace("../","")}')
                # exit()
        ...
        if response.status_code == 200:
            ...
            for playlist in playlists:
                if playlist["stream_info"]["resolution"] == self.resolution:
                    playlist_uri = F"{self.host_directory.replace('/Bali.m3u8','')}/{playlist['uri']}"
                    break
    ...
    def StartEngine(self) -> None:
        ...
        try:
            while self.start_process:
                ...
                if status_ts:
                    now_filename = F"TVRIBALISTREAMING_{datetime.datetime.now().strftime('%m-%d-%H-%M-%S')}"
...
if __name__ == "__main__":
    ...
    BeritaSatu = BeritaSatu(
        ...
        converter_host=CONFIG.SOCKET_SERVER_TVRIBALI["HOST"],
        converter_port=CONFIG.SOCKET_SERVER_TVRIBALI["PORT"],
        buffer_size=CONFIG.SOCKET_SERVER_TVRIBALI["BUFFER_SIZE"],
