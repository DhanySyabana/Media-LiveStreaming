...
from libs.VideoProsessorRtv import VideoProsessor
...
class INewsV1:
    ...
    def GetToken(self) -> str:
        ...
        token = session.post('https://www.vidio.com/live/1561/tokens', self.custom_headers)
        ...
        try:
            while self.start_process:
                if playlist_uri is not None:
                    ...
                    if status_ts:
                        now_filename = F"RTVSTREAMING_{datetime.datetime.now().strftime('%m-%d-%H-%M-%S')}"
...
if __name__ == "__main__":
    ...
    inews = INewsV1(
        ...
        converter_host=CONFIG.SOCKET_SERVER_RTV["HOST"],
        converter_port=CONFIG.SOCKET_SERVER_RTV["PORT"],
        buffer_size=CONFIG.SOCKET_SERVER_RTV["BUFFER_SIZE"],
    )
    inews.StartEngine()   
