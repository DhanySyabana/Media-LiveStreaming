...
class KompasTv:
    ...
    def _handle_error_with_notification(self, error_message: str, send_immediate: bool = True) -> None:
        ...
        if send_immediate and self.countdown_counter == 0:
            logging.error(f'[KompasTv] {{error_message}}')
    ...
    def StartEngine(self) -> None:
        ...
        try:
            while self.start_process:
                ...
                try:
                    logging.info(f"[{kompas}] Fetching stream URL...")
                    stream_url = self._get_yt_stream_url()

                    if not stream_url:
                        logging.error(f"[{kompas}] Failed to obtain stream URL; skipping iteration", extra={"log_text": "Failed to obtain stream URL"})
                        time.sleep(3)
                        continue

                    logging.info(f"[{kompas}] Recording {self.video_duration}s -> {out_path}")
                    ...
                    logging.error(f"[{kompas}] ERROR record: {e}", extra={"log_text": error_message, "detail": str(e)}, exc_info=True)
