...
from libs.VideoProsessorTvriSumut import VideoProsessor
        ...
        if response.status_code == 200:
            ...
            for playlist in playlists:
                if playlist["stream_info"]["resolution"] == self.resolution:
                    playlist_uri = F"{self.host_directory.replace('/Sumut.m3u8','')}/{playlist['uri']}"
                    break
    ...
    def StartEngine(self) -> None:
        ...
        try:
            while self.start_process:
                ...
                if status_ts:
                    now_filename = F"TVRISUMUTSTREAMING_{datetime.datetime.now().strftime('%m-%d-%H-%M-%S')}"
...
if __name__ == "__main__":
    ENGINE_NAME = "TVRISUMUTSTREAMING"
    CONFIG = Config()
    ...
    BeritaSatu = BeritaSatu(
        ...
        converter_host=CONFIG.SOCKET_SERVER_TVRISUMUT["HOST"],
        converter_port=CONFIG.SOCKET_SERVER_TVRISUMUT["PORT"],
        buffer_size=CONFIG.SOCKET_SERVER_TVRISUMUT["BUFFER_SIZE"],
