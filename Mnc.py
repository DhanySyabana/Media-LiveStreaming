...
class INewsV1:
    ...
    def GetPathSDI(self, selenium:None) -> str:
        path_uri = None
        token = requests.get('https://api.rctiplus.com/api/v1/live-event/2/url?appierid=undefined', headers={'Authorization':'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJ2aWQiOjAsInRva2VuIjoiYThjOGFlZDgyN2JjOTJjOSIsInBsIjoid2ViIiwiZGV2aWNlX2lkIjoiNjc3NTBhZTYtZmE0OC0xMWVkLTgxYWItMDAxNjNlMDM4NzFhIn0.UuAVu9RYec7Xur5X7JbnyEQ1AafZRI6Dw4wrYfYphls'})
        ...
        if response.status_code == 200:
            ...
            for playlist in playlists:
                if playlist["stream_info"]["resolution"] == self.resolution:
                    self.query = url.split('/mnctv-sdi.m3u8')[0]
                    auth = self.query.partition('rctiplus.id/')[2]
                    playlist_uri = F"{self.host_directory}/{auth}/{playlist['uri']}"
                    headers={
                        'accept': 'application/json',
                        'Authorization':'Bearer '+auth,
                        'origin': 'https://www.rctiplus.com',
    ...
    def GetSegments(self, playlist_uri:str) -> list:
        ...
        if response.status_code == 200:
            ...
            for segment in segments:
                auth = playlist_uri.split('/mnctv-sdi-')[0]
                auth = auth.partition('rctiplus.id/')[2]
                headers={
                    'accept': 'application/json',
                    'Authorization':'Bearer '+auth,
                    'origin': 'https://www.rctiplus.com',
                ...
                self.custom_headers = headers
                response = HTTPRequest("get", F"{self.host_directory}/{auth}/{segment['uri']}", self.custom_headers).Hit()
                if response.status_code == 200:
                    file_segments.append({
                        "url": F"{self.host_directory}/{auth}/{segment['uri']}",
                        "sequence": int(segment["uri"].split("seq=")[1].split(".ts")[0])
                    })
                else:
                    logging.error(F"Error Get Segments TS: {response.status_code}")
                    continue
