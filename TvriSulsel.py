        ...
        if response.status_code == 200:
            ...
            for playlist in playlists:
                if playlist["stream_info"]["resolution"] == self.resolution:
                    playlist_uri = F"{self.host_directory.replace('/Sulsel.m3u8','')}/{playlist['uri']}"
                    break
...
if __name__ == "__main__":
    ...
    BeritaSatu = BeritaSatu(
        ...
        converter_host=CONFIG.SOCKET_SERVER_TVRISULSEL["HOST"],
        converter_port=CONFIG.SOCKET_SERVER_TVRISULSEL["PORT"],
        buffer_size=CONFIG.SOCKET_SERVER_TVRISULSEL["BUFFER_SIZE"],
