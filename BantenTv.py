...
class BeritaSatu:
    ...
    def GetSegment(self,  playlist_uri:str) -> list:
        file_segments = []
        logging.info(F"Playlist URI: {playlist_uri}")
        ...
        response = requests.get(playlist_uri, verify=False, headers=self.custom_headers)
        ...
        try:
            while self.start_process:
                if playlist_uri is not None:
                    ...
                    if status_ts:
                        now_filename = F"BANTENTVSTREAMING_{datetime.datetime.now().strftime('%m-%d-%H-%M-%S')}"
...
if __name__ == "__main__":
    ...
    BeritaSatu = BeritaSatu(
        ...
        converter_host=CONFIG.SOCKET_SERVER_BANTENTV["HOST"],
        converter_port=CONFIG.SOCKET_SERVER_BANTENTV["PORT"],
        buffer_size=CONFIG.SOCKET_SERVER_BANTENTV["BUFFER_SIZE"],
