...
from libs.VideoProsessorJakTv import VideoProsessor
        ...
        try:
            while self.start_process:
                if playlist_uri is not None:
                    ...
                    if status_ts:
                        now_filename = F"JAKTVSTREAMING_{datetime.datetime.now().strftime('%m-%d-%H-%M-%S')}"
...
if __name__ == "__main__":
    ...
    idxindonesia = IDXIndonesiaV1(
        ...
        converter_host=CONFIG.SOCKET_SERVER_JAKTV["HOST"],
        converter_port=CONFIG.SOCKET_SERVER_JAKTV["PORT"],
        buffer_size=CONFIG.SOCKET_SERVER_JAKTV["BUFFER_SIZE"],
    )
