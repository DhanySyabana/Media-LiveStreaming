...
class BeritaSatu:
    ...
    def GetSegment(self) -> list:
        ...
        response = requests.get(url =F"{self.host_directory}/{self.playlist}", headers=self.custom_headers)
        ...
        if response.status_code == 200:
            ...
            for segment in segments:
                file_segments.append({
                    ...
                    "sequence": str(segment["uri"]).replace('.ts','').partition('?')[0]
                })
